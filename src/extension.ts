...
export function activate(context: vscode.ExtensionContext) {
  ...
  let disposable = vscode.commands.registerCommand(
    "folder-structure-generator.generateFromTree",
    async () => {
      ...
      if (!text.trim()) {
        vscode.window.showErrorMessage(
          "No tree structure found! Please select or provide a folder tree structure."
        );
        return;
      }
      ...
      try {
        ...
        const choice = await vscode.window.showWarningMessage(
          `This will create ${itemCount} items in your workspace. Continue?`,
          "Yes",
          "No"
        );
        ...
        vscode.window.showInformationMessage(
          `Successfully created folder structure with ${itemCount} items!`
        );
      } catch (error: any) {
        vscode.window.showErrorMessage(
...
function parseTreeStructure(treeText: string): FileSystemItem[] {
  const lines = treeText.split("\n").filter((line) => line.trim());
  const root: FileSystemItem[] = [];
  const stack: { item: FileSystemItem; depth: number }[] = [];
  ...
  for (const line of lines) {
    ...
    const isFolder =
      content.endsWith("/") ||
      (!content.includes(".") &&
        !["php", "css", "js", "html", "json", "txt", "md"].some((ext) =>
    ...
    console.log(
      `Parsed: "${line}" -> Name: "${item.name}", Depth: ${depth}, Type: ${item.type}`
    );
...
function countItems(structure: FileSystemItem[]): number {
  let count = 0;

  function countRecursive(items: FileSystemItem[]) {
